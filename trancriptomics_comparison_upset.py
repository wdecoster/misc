...
def main():
    ...
    annot = pd.read_csv("d4905.flair.isoforms.sqantiannot.txt", sep="\t", usecols=["isoform", "gene", "structural_category", "exons"])
    annot["identifier"] = annot["isoform"].str.cat(annot['gene'], sep="_")
    annot["multi-exonic"] = annot["exons"].map(lambda x: "multi-exonic" if x > 1 else "mono-exonic")
    ...
    with open("comparison_plots.html", 'w') as output:
        ...
        df2 = df[(df["structural_category"] != "intergenic") & (df["exons"] != 1)]
        for min_count in [0, 5, 10]:
            usp = UpSetPlotly(samples=[get_transcripts(df2, c, min_count) for c in datasets],
                              sample_names=datasets
                              )
            html = usp.plot(order_by='decreasing', show_fig=False, return_fig=True) \
                 .update_layout(title=f"Number of multi-exonic non-intergenic transcripts detected by more than {min_count} supporting reads") \
                 .to_html(full_html=True, include_plotlyjs='cdn')
            output.write(html)
